...
class MaximizeProgressTaskCollisionInfluenceTimeLimit(Task):
    ...
    def reward(self, agent_id, state, action) -> float:
        ...
        wrong_way = agent_state['wrong_way']
        wall_collision = agent_state['wall_collision']
        steering_action = np.clip(action['steering_competition'], -1, 1)
        velocity = agent_state['velocity']
        velocity_x = float(velocity[0]) 
        velocity_y = float(velocity[1])
        velocity_car = np.sqrt(pow(velocity_x, 2) + pow(velocity_y, 2))
        motor_action = np.clip(action['motor_competition'], -1, 1)
        ...
        # 進度獎勵
        progress_diff = progress - self._last_stored_progress
        if progress_diff > 0:
            reward += 1000*progress_diff
        else:
            reward -= 0.1

        # 檢查點獎勵
        if checkpoint > self._previous_checkpoint:
            reward += 10

        # 平稳驾驶惩罚：基于速度和转向的剧烈变化
        steering_diff = abs(steering_action - self._last_action['steering_competition'])
        reward -= 0.1 * steering_diff
